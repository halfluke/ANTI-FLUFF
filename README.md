...
**Admin panel** (methodology/section/check editor with **fluffy vs exploitable** slider and bar under *Add Check*; **Write package-state.json** only in `pnpm run dev`):
...
pnpm install    # once, or when dependencies change
pnpm run dev    # open the URL Vite prints (often http://localhost:5173)
```

**Ctrl+C** stops the server. You only need **`pnpm install`** again after dependency changes.
...
Upload **`dist/`** to any static host (nginx, S3+CloudFront, Netlify, internal share, …). **No Node or Vite on the server** — only static files. `pnpm run preview` is optional smoke-test before upload.
...
| | **`pnpm run dev`** | **Production** (`dist/` on a static host) |
|--|--|--|
| **Disk: `src/data/package-state.json`** | **Manual only:** Admin → **Write package-state.json now** (POST to Vite during `pnpm run dev`). No automatic disk writes. | **Never** — file is whatever was **bundled at build time**. |
...
| **Typical use** | Portable backup, hand-edit **`src/data/*.json`**, import on another browser. | Snapshot **whole** in-app state into git (methodologies **and** ticks/selection) from **`pnpm run dev`**. |
...
| **Write package-state.json now** | **Admin** only when **`MODE === 'development'`** (usually **`pnpm run dev`**). Hidden in **`dist/`**; static hosts have no save endpoint. |
